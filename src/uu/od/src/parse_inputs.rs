...
/// POSIX ERANGE value, used for overflow error messages.
/// Defined here because `ERANGE` is not available on all targets (e.g. WASM).
        ...
        if !matches.opts_present(&[
            ...
            match offset {
                ...
                Err(e) => {
                    ...
                    let err = std::io::Error::from_raw_os_error(ERANGE);
